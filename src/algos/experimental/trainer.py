...
import numpy as np
import random
from algos.experimental.networks import RNNAgent, MixingNetworkExperimental
from core.utils.episode_replay_buffer import EpisodeReplayBuffer


class QMIXTrainerExperimental:
    """
    ...
    def __init__(self, env, n_agents, state_dim, obs_dim, n_actions,
        ...
        # Mixer
        self.mixer = MixingNetworkExperimental(n_agents, state_dim, mixing_hidden_dim).to(self.device)
        self.target_mixer = MixingNetworkExperimental(n_agents, state_dim, mixing_hidden_dim).to(self.device)
