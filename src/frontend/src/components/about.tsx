    ...
    <div className="bg-slate-800/40 backdrop-blur-md border border-slate-700/50 p-6 md:p-8 rounded-3xl shadow-xl w-full relative overflow-hidden mb-12">
      ...
      <div className="w-full overflow-x-auto relative z-10">
        <div className="min-w-[1000px]"> 
          <svg viewBox="0 0 1100 600" className="w-full h-auto">
            {drawEdges(200, 425, 'in', 0, 3, 4)}   
            {drawEdges(425, 650, 'h1', 12, 4, 4)}  
            {drawEdges(650, 875, 'h2', 28, 4, 4)}  

            {drawRecurrentLoops(425, 'h1', 0)}
            {drawRecurrentLoops(650, 'h2', 4)}
            ...
            {Array.from({ length: 4 }).map((_, j) => (
              <g key={`h1-${j}`} onMouseEnter={() => setHoveredNode({layer: 'h1', index: j})} onMouseLeave={() => setHoveredNode(null)} className="cursor-pointer">
                {isLearning && activeNodes.h1.includes(j) && <circle cx={425} cy={110 + j * 125} r={32} fill="#94a3b8" opacity={0.4} className="animate-pulse" />}
                <circle cx={425} cy={110 + j * 125} r={22} fill="#64748b" stroke={hoveredNode?.layer === 'h1' && hoveredNode.index === j ? '#ffffff' : '#0f172a'} strokeWidth={2} className="transition-all drop-shadow-md" />
                <text x={425} y={110 + j * 125 + 38} textAnchor="middle" fill="#94a3b8" fontSize={12} fontFamily="monospace" fontWeight="bold">b: {biases[j].toFixed(2)}</text>
              </g>
            ))}

            {Array.from({ length: 4 }).map((_, j) => (
              <g key={`h2-${j}`} onMouseEnter={() => setHoveredNode({layer: 'h2', index: j})} onMouseLeave={() => setHoveredNode(null)} className="cursor-pointer">
                {isLearning && activeNodes.h2.includes(j) && <circle cx={650} cy={110 + j * 125} r={32} fill="#94a3b8" opacity={0.4} className="animate-pulse" />}
                <circle cx={650} cy={110 + j * 125} r={22} fill="#64748b" stroke={hoveredNode?.layer === 'h2' && hoveredNode.index === j ? '#ffffff' : '#0f172a'} strokeWidth={2} className="transition-all drop-shadow-md" />
                <text x={650} y={110 + j * 125 + 38} textAnchor="middle" fill="#94a3b8" fontSize={12} fontFamily="monospace" fontWeight="bold">b: {biases[4+j].toFixed(2)}</text>
              </g>
            ))}

            {OUTPUT_LABELS.map((label, k) => (
              <g key={`out-${k}`} onMouseEnter={() => setHoveredNode({layer: 'out', index: k})} onMouseLeave={() => setHoveredNode(null)} className="cursor-pointer">
                {isLearning && activeNodes.out.includes(k) && <circle cx={875} cy={110 + k * 125} r={32} fill="#10b981" opacity={0.4} className="animate-pulse" />}
                <circle cx={875} cy={110 + k * 125} r={22} fill="#10b981" stroke={hoveredNode?.layer === 'out' && hoveredNode.index === k ? '#ffffff' : '#0f172a'} strokeWidth={2} className="transition-all drop-shadow-md" />
                <text x={910} y={110 + k * 125} textAnchor="start" alignmentBaseline="middle" fill={hoveredNode?.layer === 'out' && hoveredNode.index === k ? '#10b981' : '#f8fafc'} fontSize={15} fontWeight="bold" className="transition-colors">{label}</text>
                <text x={875} y={110 + k * 125 + 38} textAnchor="middle" fill="#94a3b8" fontSize={12} fontFamily="monospace" fontWeight="bold">b: {biases[8 + k].toFixed(2)}</text>
              </g>
            ))}
            ...
            <text x={425} y={30} textAnchor="middle" fill="#cbd5e1" fontSize={14} fontWeight="bold" className="uppercase tracking-widest">Hidden Layer 1 (Recurrent)</text>
            <text x={650} y={30} textAnchor="middle" fill="#cbd5e1" fontSize={14} fontWeight="bold" className="uppercase tracking-widest">Hidden Layer 2 (Recurrent)</text>
            <text x={875} y={30} textAnchor="middle" fill="#cbd5e1" fontSize={14} fontWeight="bold" className="uppercase tracking-widest">Action Logits</text>
          </svg>
        </div>
      </div>
    </div>
    ...
    <div className="w-full flex flex-col items-center">
      <div className="flex flex-col items-center mb-12 relative z-10 w-full max-w-4xl bg-slate-800/60 backdrop-blur-xl border border-slate-700/50 p-8 rounded-3xl shadow-2xl">
        ...
        <button 
          ...
          className={`px-10 py-4 rounded-xl font-black tracking-widest text-sm uppercase transition-all duration-300 ${
            isLearning 
